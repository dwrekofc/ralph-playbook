...
3 Codex subagents: Generator (builds), Evaluator (quality gate), Documentor (traces everything). You are the lead orchestrating all three.
...
All ≥90% → skip to Phase 2. Any <90% → ask the user via `request_user_input`. Max 5 questions. Only ask about what and why, never how. Batch into one call.
...
**IMPORTANT: Use Codex subagents.** Spawn separate generator, evaluator, and documentor agents so they have independent contexts and file ownership. Use GPT-5.5 for all three; use reasoning effort high for evaluator judgment and difficult generator/debugging work, and medium for straightforward implementation/search work.

Create 3 subagents. Give each their role, the task context from Phase 0, and the back-pressure commands from Phase 2.
...
Present results to user via `request_user_input`:
