...
- **Use `AskUserQuestion` for every decision** — variant, desc, flags, next steps. Never assume.
...
3. Use `AskUserQuestion` for git/GitHub preferences:
...
5. After success, highlight the reverse-engineering option:
   ...
   - `./loop.sh reverse` — traces your code and produces implementation-free specs
...
- **Don't run `./loop.sh reverse` without asking** — it's a long-running headless process. Always confirm first.
