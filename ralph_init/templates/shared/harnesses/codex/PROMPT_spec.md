...
0c. Read all existing `specs/*.md` files (if any) with up to 250 parallel Codex subagents with reasoning effort medium to understand current spec coverage.

0d. If source code exists in `src/` or `crates/` or `apps/`, read it with up to 250 parallel Codex subagents with reasoning effort medium to understand what has been implemented.
...
999999999999. This prompt runs AUTONOMOUSLY in a headless loop. Do NOT use request_user_input or any interactive tools. Do NOT stop and wait for user input. Complete the full cycle (orient → refine → quality pass → commit) every iteration.
