...
If the user's goal is missing important intent, ask concise questions with `AskUserQuestion`. Ask only for choices that materially change the thread.
...
Ask which optional enhancements to include with `AskUserQuestion`. Useful options:
...
Before writing anything, present the Thread Brief and ask for approval with `AskUserQuestion`.
...
- Use Claude Code headless execution when it calls Claude: `claude -p --dangerously-skip-permissions --output-format=stream-json --model opus --verbose`
