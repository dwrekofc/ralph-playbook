<!-- description: Cleanroom research — read sources, write reviewable behavior notes -->
...
The notes you produce are reviewable artifacts. A human reads them and corrects mistakes before any spec or build phase begins. After review, those notes feed into `/ralph-reqs` → `/ralph-spec` → `./loop.sh auto` (or `/ralph-rapid-prototype` → `./loop.sh auto`).
...
Use up to 100 parallel Sonnet subagents for reading. Use Opus subagents for complex tracing or cross-source reconciliation.
...
9999999999999. **This prompt runs AUTONOMOUSLY in a headless loop.** Do NOT use `AskUserQuestion` or any interactive tools. Do NOT stop and wait for user input. Complete the research pass every iteration.
